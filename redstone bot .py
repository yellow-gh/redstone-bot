...
async def on_raw_reaction_add(payload):
    ...
    if payload.channel_id == 789478285205569559:
        ...
        if not payload.member == mem:
            global teilnehmer
            teilnehmer.append(payload.member)
...
async def on_member_join(member):
    ...
    if not member.bot:
        embed = discord.Embed(title="Willkommen auf yellow_redstone's discord {} ".format(member.name),description='von redstone bot', color=0x22a7f0)
        embed.add_field(name="Hallo auch von meiner seite, ich passe auf das du keinen Mist baust. Aber nun viel spaß auf yellow_redstone's discord",value='** **',inline=True)
        try:
...
async def change_status():
    ...
    await channel.edit(name='Memercount : {}'.format(guild.member_count))
...
async def help(ctx):
    if ctx.author.bot == False:
        ...
        embed.add_field(name='**reset** - resetet das Gewinnspiel',value='** **',inline=True)
...
async def add(ctx, *, arg):
    embed = discord.Embed(title='**Gewinnspiel**',color=0x22a7f0)
...
async def add_error(ctx, error):
    if isinstance(error, commands.MissingPermissions):
        embed = discord.Embed(title='**Error** ',color=0x22a7f0)
...
async def chose(ctx):
    embed = discord.Embed(title='**Gewinnspiel**',color=0x22a7f0)
    embed.add_field(name='{} hat gewonnen'.format(random.choice(teilnehmer)),value='** **',inline=True)
    await ctx.send(embed=embed)
...
async def play(ctx, *, url):
    ...
    if not voice_channel.is_playing():
        ...
        embed = discord.Embed(title='**Spielt jetzt:** ', description='**{}**'.format(player.title),color=0x22a7f0)
...
async def userinfo(ctx, member : discord.Member):
    embed = discord.Embed(title='Userinfo für {}'.format(member.name),description='Dies ist eine Userinfo für den User {}'.format(member.mention),color=0x22a7f0)
...
async def clear(ctx, amount=1):
    ...
    embed = discord.Embed(title='**Clear**', description='{0} Nachrichten wurden gelöscht'.format(amount),color=0x22a7f0)
    embed.set_footer(text=f'angefordert von {ctx.author}')
...
async def ball(ctx, *, arg):
    ...
    embed = discord.Embed(title='**8ball**', description='Ich kontaktiere das Orakel...',color=0x22a7f0)
    embed.set_footer(text=f'angefordert von {ctx.author}')
    await ctx.send(embed=embed, delete_after=2.0)
    await asyncio.sleep(2)
    embed = discord.Embed(title='**8ball**', description='Deine Antwort zur Frage `{0}` lautet: `{1}`'.format(arg, random.choice(antworten)),color=0x22a7f0)
    embed.set_footer(text=f'angefordert von {ctx.author}')
    await ctx.send(embed=embed)
...
async def ball_error(ctx, error):
    if isinstance(error, commands.MissingRequiredArgument):
        embed = discord.Embed(title='**Error** ', description='',color=0x22a7f0)
...
async def temp(ctx):
    ...
    dateilesen.close()
    embed = discord.Embed(title='Temperatur', description='Die CPU hat ' + temperatur + ' Grad',color=0x22a7f0)
    embed.set_footer(text=f'angefordert von {ctx.author}')
    await ctx.send(embed=embed)
